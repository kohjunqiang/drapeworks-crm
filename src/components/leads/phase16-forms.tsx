...
function useSubmit(action: (value: Record<string, FormDataEntryValue>) => Promise<unknown>, onSuccess?: () => void | Promise<void>) {
  const [pending, start] = useTransition();
  const router = useRouter();
  return { pending, submit: (form: HTMLFormElement) => start(async () => { try { await action(Object.fromEntries(new FormData(form))); toast.success("Saved"); await onSuccess?.(); router.refresh(); } catch (error) { toast.error(error instanceof Error ? error.message : "Save failed"); } }) };
}
