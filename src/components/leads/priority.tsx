...
export function PriorityBadge({ lead, compact = false }: { lead: PriorityInput & StoredPriority; compact?: boolean }) {
  ...
  if (compact) return <span title={result.priority_class ? `${result.priority_class} — ${PRIORITY_MEANINGS[result.priority_class].label}` : `Missing: ${result.missing.join(", ")}`} className={`inline-flex whitespace-nowrap rounded px-2 py-0.5 text-xs font-semibold ${result.priority_class ? styles[result.priority_class] : "bg-slate-100 text-slate-500"}`}>{result.priority_class ? `${result.priority_class} · ${result.priority_score}` : "Pending"}</span>;
