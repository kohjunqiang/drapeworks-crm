...
describe("Phase 16 funnel engine",()=>{
  ...
  it.each([["Customer Replied","Reply Required"],["No Response","Follow-Up"],["Pre-Appointment Barrier","Resolve Appointment Barrier"],["Appointment Booked","Confirm / Attend Appointment"],["Quotation Sent","Push for Deposit"],["Post-Appointment Barrier","Resolve Closing Barrier"],["Customer Declined","Closed"],["Customer Confirmed","Won"]] as const)("maps outcome %s",(o,x)=>expect(deriveActionRequired(lead("Activate Lead – Short Term",o),today)).toBe(x));
  ...
  it.each([["2026-08-30","Awaiting Customer"],["2026-08-29","Follow-Up"],["2026-08-28","Follow-Up"],[null,"Follow-Up"]] as const)("date-gates awaiting customer",(d,x)=>expect(deriveActionRequired(lead("Activate Lead – Short Term","Awaiting Customer",{next_action_date:d}),today)).toBe(x));
  ...
  it("terminal outcome gets a closing recommendation",()=>expect(deriveRecommendations(lead("Decision Pending","Customer Confirmed"),today)).toContainEqual(expect.objectContaining({code:"customer-confirmed",suggestedStage:"Won"})));
});
