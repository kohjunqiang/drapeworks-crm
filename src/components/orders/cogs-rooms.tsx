 ...
 * mesh panels) indented beneath, each named by what it's made of, then the
 * order-level lines (the rails) that belong to no single window.
 *
 * Emits bare <dt>/<dd> rows — the caller owns the <dl> and the freight / other
 * cost / GST rows that follow, since those differ per surface.
 */
      ...
      {visibleCogsRooms(rooms).map((room, roomIdx) => (
        <Fragment key={roomIdx}>
          ...
          {room.items.map((item, itemIdx) => (
            <div
              key={itemIdx}
              className="flex justify-between gap-2 pl-3 text-slate-400"
            >
