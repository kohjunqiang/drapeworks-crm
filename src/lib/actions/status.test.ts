...
function setupOrder(currentStatus = "fulfilment") {
  const values = vi.fn();
  const select = {
    select: () => select,
    where: () => select,
...
describe("order completion payment confirmation", () => {
  it.each([undefined, false, "true", 1])("rejects completion with confirmation %s", async (confirmation) => {
    ...
    await expect(advanceOrderStatus({ orderId, expectedStatus: "fulfilment", balanceReceivedConfirmed: confirmation })).rejects.toThrow();
  ...
  it("records explicit receipt confirmation and the operator in the completion event", async () => {
    ...
    await advanceOrderStatus({ orderId, expectedStatus: "fulfilment", balanceReceivedConfirmed: true, note: " Paid by PayNow " });
  ...
  it("preserves confirmation when no optional note was supplied", async () => {
    const { values } = setupOrder();
    await advanceOrderStatus({ orderId, expectedStatus: "fulfilment", balanceReceivedConfirmed: true });
  ...
  it("still rejects stale submissions even with receipt confirmation", async () => {
    ...
    await expect(advanceOrderStatus({ orderId, expectedStatus: "fulfilment", balanceReceivedConfirmed: true })).rejects.toThrow("already changed");
    expect(trx.insertInto).not.toHaveBeenCalled();
  });
