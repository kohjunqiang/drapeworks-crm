...
export async function WorkView({ params, userId }: { params: Record<string, string | undefined>; userId: string }) {
  ...
  const pageSizeHref = (size: number) => { const next = new URLSearchParams(Object.entries(params).filter((entry): entry is [string, string] => entry[1] !== undefined && !entry[0].endsWith("Page"))); next.set("view", "work"); next.set("pageSize", String(size)); return `/leads?${next.toString()}`; };
  ...
  return <>
    ...
    {GROUPS.map(group => {
      ...
      return <section key={group} className="mb-8"><div className="mb-3 flex items-center justify-between"><h2 className="flex items-center gap-2 font-semibold"><span className={`rounded-full border px-2.5 py-1 text-xs font-semibold ${GROUP_STYLES[group]}`}>{GROUP_LABELS[group]}</span><span className="text-sm text-slate-500">{allItems.length}</span></h2>{pageCount > 1 && <nav aria-label={`${group} top pagination`} className="flex items-center gap-1"><Link aria-disabled={page === 1} tabIndex={page === 1 ? -1 : undefined} className={`inline-flex h-8 items-center justify-center rounded-lg border px-2.5 text-xs font-medium ${page === 1 ? "pointer-events-none border-slate-200 text-slate-300" : "border-slate-200 bg-white text-slate-700 hover:bg-slate-50"}`} href={pageHref(key, Math.max(1, page - 1))}>Previous</Link><span className="px-1 text-xs text-slate-500">Page {page} of {pageCount}</span><Link aria-disabled={page === pageCount} tabIndex={page === pageCount ? -1 : undefined} className={`inline-flex h-8 items-center justify-center rounded-lg border px-2.5 text-xs font-medium ${page === pageCount ? "pointer-events-none border-slate-200 text-slate-300" : "border-slate-200 bg-white text-slate-700 hover:bg-slate-50"}`} href={pageHref(key, Math.min(pageCount, page + 1))}>Next</Link></nav>}</div>
    ...
    <div className="mt-2 flex items-center justify-end gap-1 text-xs text-slate-500"><span className="mr-1">Rows per table</span>{[10,25,50].map(size => <Link key={size} href={pageSizeHref(size)} className={`rounded-md px-2 py-1.5 font-medium ${pageSize === size ? "bg-slate-900 text-white" : "hover:bg-slate-100"}`}>{size}</Link>)}</div>
  </>;
}
