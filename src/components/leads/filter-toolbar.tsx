...
import { FILTER_DATES, FILTER_SELECTS, selectedFilters, columnFilterPills } from "@/lib/leads/workspace-filters";
    ...
    <div className="flex h-10 items-center justify-between gap-3"><span className="min-w-0 truncate text-sm font-medium text-slate-700">{view === "all" && !filters.length ? "No filters applied" : "Applied filters"}</span>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        ...
        <DialogContent className="sm:max-w-2xl">
          ...
          <form onSubmit={event => { event.preventDefault(); const data = new FormData(event.currentTarget); const next = new URLSearchParams(); for (const [key, value] of data) if (typeof value === "string" && value.trim()) next.set(key, value.trim()); next.set("view", view); next.set("pageSize", String(pageSize)); router.push(`/leads?${next.toString()}`); setOpen(false); }}>
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              ...
              {FILTER_SELECTS.map(field => <label key={field.key} className="min-w-0 text-sm">{field.label}<select name={field.key} defaultValue={params[field.key] ?? ""} className={control}><option value="">All</option>{field.values.filter(value => view !== "work" || field.key !== "stage" || (value !== "Lost" && value !== "Not Qualified")).map(value => <option key={value} value={value}>{value === "Curtains / Blinds" ? "Curtains & Blinds" : value === "Both" ? "Both (legacy)" : value}</option>)}</select></label>)}
