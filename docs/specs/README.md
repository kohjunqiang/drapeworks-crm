...
| 14 | `phase-14-window-addons.md` | **Window add-ons & blinds-only toilets** — replaces the hard-coded `add_s_fold`/`add_slim_tracks` columns with an admin-maintained add-on list (`applies_to` scoping, auto-apply rules) joined to windows, wires blinds up to add-ons for the first time (Blackout, plus Extra shipping forced over 200 cm width), and retires the toilet-curtain variant so a toilet window is a blind. **Specified 2026-08-21, not implemented.** |
