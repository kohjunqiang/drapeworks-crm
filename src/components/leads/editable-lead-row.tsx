...
export function EditableLeadRow({ lead, consultants, variant, ownerName, actionLabel, moveInDisplay, lastContactText, dueLabel }: {
  ...
  return variant === "work" ? <tr className="border-b last:border-b-0 hover:bg-slate-50/70"><td className="px-2 py-3"><QuickEditLead lead={lead} consultants={consultants} trigger="name"/><div className="text-xs text-slate-500">{lead.lead_ref}</div></td><td className="px-2 py-3 font-medium">{actionLabel}</td><td className="px-2 py-3">{lead.funnel_stage}</td><td className="max-w-64 p-3 text-slate-600">{lead.action_detail ?? "—"}</td><td className="px-2 py-3">{lead.next_action_date_text ?? "—"}</td><td className="px-2 py-3">{moveInDisplay ?? "—"}</td><td className="px-2 py-3">{lead.latest_quote_cents ? `$${(lead.latest_quote_cents / 100).toLocaleString("en-SG", { minimumFractionDigits: 2 })}` : "—"}</td><td className="px-2 py-3">{lastContactText ?? "—"}</td><td className="px-2 py-3">{ownerName}</td><td className="px-2 py-3">{actions}</td></tr>
    : <tr className="border-b last:border-b-0 hover:bg-slate-50/70">
      ...
      {[lead.inbound_outbound, lead.initiated_date_text, lead.last_contact_date_text, lead.funnel_stage, lead.lead_status, lead.last_outcome, actionLabel, lead.action_detail, lead.next_action_date_text, dueLabel].map((value, index) => <td key={index} className="truncate px-2 py-2" title={value ?? undefined}>{value ?? "—"}</td>)}
