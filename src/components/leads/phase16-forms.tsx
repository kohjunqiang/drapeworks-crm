...
import { BUYING_STAGES, ENGAGEMENT_QUALITIES, ENGAGEMENT_HELP, type PriorityInput } from "@/lib/leads/priority";
import { PriorityBreakdown } from "@/components/leads/priority";

function PriorityFields({ lead = {} }: { lead?: PriorityInput }) {
  return <fieldset className="space-y-3 rounded-xl border border-slate-200 p-4"><legend className="px-1 text-sm font-semibold">Lead Priority</legend><div className="grid gap-4 sm:grid-cols-2">
    <Field label="Renovation / Buying Stage" hint="Select the furthest stage reached. This is not cumulative and is not changed automatically by the move-in date."><Select name="renovation_buying_stage" defaultValue={lead.renovation_buying_stage}><option value="">Not assessed</option>{options(BUYING_STAGES)}</Select></Field>
    <Field label="Engagement Quality" hint="Your judgement of the conversation quality."><Select name="engagement_quality" defaultValue={lead.engagement_quality}><option value="">Not assessed</option>{options(ENGAGEMENT_QUALITIES)}</Select></Field>
    <Field label="Order / Latest Quotation Value (SGD)"><Input name="latest_quote_sgd" type="number" min="0" max="1000000" step="0.01" defaultValue={lead.latest_quote_cents == null ? "" : lead.latest_quote_cents / 100}/></Field>
  </div><details className="text-xs text-slate-600"><summary className="cursor-pointer py-2">Engagement definitions</summary>{ENGAGEMENT_QUALITIES.map(value => <p key={value} className="mt-2"><strong>{value}:</strong> {ENGAGEMENT_HELP[value]}</p>)}</details><p className="text-xs text-slate-500">All three inputs are needed. Scores update when you save; blank fields remain Pending.</p></fieldset>;
}
...
const stageOptions = (current: FunnelStage) => <>{FUNNEL_STAGES.map(value => <option key={value} value={value} disabled={value === "Won" && current !== "Won"}>{value}</option>)}</>;
const WON_HINT = "Won is set automatically when the linked order reaches Deposit Received.";

function useSubmit<Success extends { ok: true }>(action: (value: Record<string, FormDataEntryValue>) => Promise<Success | { ok: false; error: string }>, onSuccess?: (result: Success) => void | Promise<void>, successMessage = "Saved") {
  const [pending, start] = useTransition();
  const router = useRouter();
  return { pending, submit: (form: HTMLFormElement) => start(async () => { try { const result = await action(Object.fromEntries(new FormData(form))); if (!result.ok) { toast.error(result.error); return; } toast.success(successMessage); await onSuccess?.(result); router.refresh(); } catch { toast.error("Save failed. Please try again."); } }) };
}
...
export function QuickEditLead({ lead: initialLead, consultants, fullWidth = false, trigger = "edit" }: { lead: { inbound_outbound?: string | null; created_date_text?: string; initiated_date_text?: string | null; last_contact_date_text?: string | null; id: string; name: string; funnel_stage: FunnelStage; lead_status?: string; mobile?: string | null; development?: string | null; contact_channel?: string; source?: string | null; primary_product?: string | null; next_action_date_text: string | null; move_in_date_text?: string | null; action_detail: string | null; latest_quote_cents?: number | null; closure_reason?: string | null; assigned_consultant_id: string | null; owner_id: string | null }; consultants: { id: string; full_name: string | null }[]; fullWidth?: boolean; trigger?: "view" | "edit" | "name" | "advanced" }) {
  ...
  const refreshDetail = async () => { const data = await getLeadModalData(initialLead.id); setDetailData(data); setStage(data.lead.funnel_stage); setDirty(false); };
...
["Funnel Readiness", detailData?.lead.buying_readiness ?? "—"],
...
<section className="rounded-xl border border-slate-200 p-4"><h3 className="mb-3 text-sm font-semibold">Appointment</h3>{detailData?.appointment ? <><AppointmentCard key={`${detailData.appointment.id}-${new Date(detailData.appointment.updated_at).toISOString()}`} appointment={detailData.appointment} calendarConfigured={detailData.calendarConfigured} onChanged={refreshDetail}/>{detailData.appointment.status !== "scheduled" && (lead.funnel_stage === "Book Appointment" || lead.funnel_stage === "Attend Appointment") ? <div className="mt-3"><BookAppointmentDialog leadId={lead.id} leadName={lead.name} leadMobile={lead.mobile ?? null} development={lead.development ?? null} consultants={detailData.consultants} onBooked={refreshDetail}/></div> : null}</> : lead.funnel_stage === "Book Appointment" || lead.funnel_stage === "Attend Appointment" ? <BookAppointmentDialog leadId={lead.id} leadName={lead.name} leadMobile={lead.mobile ?? null} development={lead.development ?? null} consultants={detailData?.consultants ?? consultants} onBooked={refreshDetail}/> : <p className="text-sm text-slate-500">Move this lead to Book Appointment and save before booking.</p>}</section>
...
export function LogUpdateForm({ lead, onSuccess, onDirty }: { onDirty?: () => void; onSuccess?: () => void | Promise<void>; lead: { updated_at: Date | string; next_action_date_text?: string | null; closure_reason?: string | null; id: string; funnel_stage: FunnelStage; interaction_summary: string | null; action_detail: string | null; quote_valid_days: number } }) {
  ...
  return <form onChange={onDirty} onSubmit={event => { event.preventDefault(); state.submit(event.currentTarget); }} className="space-y-5">
    ...
    <div className="grid gap-4 sm:grid-cols-2">
      ...
      <Field label="Funnel stage" hint={WON_HINT}><select name="funnel_stage" value={stage} onChange={event => setStage(event.target.value as FunnelStage)} className={controlClass}>{stageOptions(stage)}</select></Field>
...
export function RecommendationBanner({ leadId, recommendation }: { leadId: string; recommendation: Recommendation }) {
  ...
  const run = (accept: boolean) => start(async () => { try { const result = await (accept ? acceptRecommendation : dismissRecommendation)({ lead_id: leadId, code: recommendation.code, ...(reason ? { closure_reason: reason } : {}) }); if (!result.ok) { toast.error(result.error); return; } toast.success(accept ? "Recommendation applied" : "Dismissed"); } catch { toast.error("Failed. Please try again."); } });
