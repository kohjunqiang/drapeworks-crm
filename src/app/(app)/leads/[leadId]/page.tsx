...
export default async function Page({ params }: { params: Promise<{ leadId: string }> }) {
  ...
  return <main className="mx-auto max-w-5xl px-4 py-6 sm:px-6 sm:py-8">
    ...
    <section className="mb-4 rounded-xl border border-slate-200 bg-white p-4 shadow-sm sm:p-6"><h2 className="mb-4 font-semibold">Appointment</h2>{appointment ? <AppointmentCard key={`${appointment.id}-${new Date(appointment.updated_at).toISOString()}`} appointment={appointment} calendarConfigured={isCalendarConfigured()}/> : lead.funnel_stage === "Attend Appointment" ? <BookAppointmentDialog leadId={id} leadName={lead.name} leadMobile={lead.mobile} development={lead.development} consultants={consultants}/> : <p className="text-sm text-slate-500">Move this lead to Attend Appointment before booking.</p>}</section>
