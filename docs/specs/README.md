...
| 13C | `phase-13c-procurement-po.md` | **Procurement PO (采购订单)** — generates the Chinese purchase order per vendor from the frozen manufacturing measurements, in metres, with fabric length or SQM derived. Written from three real POs in `resource/documents/`. **Specified 2026-08-18, not implemented**; blocked on vendor details and the factory-facing Chinese labels listed in its §8. |
