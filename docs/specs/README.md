...
| 11 | `phase-11-mesh-product-line.md` | **Mesh product line** — window mesh (AirGuard/PetGuard/MaxGuard) as a second product. `orders.product_line` discriminator + separate `mesh_panels` line items; priced per m² by category, rounded up per panel to 0.1 m², plus colour surcharge. ✅ Implemented. |
...
| 13 | `phase-13-order-flow-and-manufacture.md` | **Order flow & manufacturing measurements** — renames `order_made` → `order_recorded` and inserts `deposit_received` + `sent_to_vendor`; adds an editable `order_reference`; adds per-product-line manufacturing allowances and a measured-vs-manufacturing reconciliation screen that freezes the dimensions sent to the vendor, locks the order, and costs COGS off them. **13A and 13B implemented and verified 2026-08-18**; 13C is now its own spec, below. |
| 13C | `phase-13c-procurement-po.md` | **Procurement PO (采购订单)** — generates the Chinese purchase order per vendor from the frozen manufacturing measurements, in metres, with fabric length or SQM derived. Written from three real POs in `resource/documents/`. **Implemented 2026-08-18; generation, storage upload, preview/regenerate dialogs and supersede all verified end-to-end 2026-08-22.** It generates today for orders whose rooms and vendors are labelled. The §8 open items are business data, not code: room types beyond Living Room / Master Bedroom / Bedroom have no Chinese name, and six of eight vendors have no `internal_ref` / `name_cn` / `phone`. Generation refuses by name when a label is missing, so the gap is loud rather than silent. |
| 14 | `phase-14-window-addons.md` | **Window add-ons & blinds-only toilets** — replaces the hard-coded `add_s_fold`/`add_slim_tracks` columns with an admin-maintained add-on list (`applies_to` scoping, auto-apply rules) joined to windows via `window_addons`, wires blinds up to add-ons for the first time (Blackout, plus Extra shipping forced over 200 cm width), and retires the toilet-curtain variant so a toilet window is a blind. **Implemented 2026-08-22** — 489 tests passing; verified E2E against the live database (quote, PO, manufacture grid, edit round-trip). |
| 15 | `phase-15-leads-and-appointments.md` | **Leads, Daily Queue & appointments** — ports Alan's 244-lead spreadsheet funnel engine into the CRM (three typed fields, six values derived at read time because every rule depends on `TODAY()`), adds the appointment record the sheet never had, syncs it to a shared Google Calendar, and closes the seam between a booking and `/orders/new`. **Implemented 2026-08-25** — 842 tests, including a 248-test parity suite that pins the engine to the spreadsheet's own cached formula output. Three spreadsheet bugs are ported deliberately so the two can be diffed; the pipeline-total bug is the one fixed. Calendar auth is an OAuth refresh token, not a service account — the Cloud org blocks service-account keys. Four E2E checks remain open (no-show rollback, the `ops` lockout, the Google-hang drill, and retiring the spreadsheet); see the spec's status block. |
| 17 | `phase-17-editable-quotations.md` | **Editable quotations until deposit** — a sent quotation stays editable until the deposit converts it to a Zoho invoice; edits update the same Zoho estimate in place (QT number never changes), refresh the PDF and write an append-only version snapshot. Confirm-sent happens once. Every quotation action returns readable errors instead of the production digest. Approved design 2026-09-24. |
