...
export async function WorkView({ params, userId }: { params: Record<string, string | undefined>; userId: string }) {
  ...
  return <>
    ...
    <div className="mb-6 flex items-center justify-between gap-3"><p className="text-sm text-slate-500">Prioritized by urgency, stage, next date, and quote value.</p><div className="inline-flex shrink-0 rounded-lg border border-slate-200 bg-white p-1 shadow-sm"><Link href="/leads?view=work" className={`min-w-16 rounded-md px-3 py-2 text-center text-sm font-medium ${owner !== "team" ? "bg-slate-900 text-white shadow-sm" : "text-slate-600 hover:bg-slate-100"}`}>Mine</Link><Link href="/leads?view=work&owner=team" className={`min-w-16 rounded-md px-3 py-2 text-center text-sm font-medium ${owner === "team" ? "bg-slate-900 text-white shadow-sm" : "text-slate-600 hover:bg-slate-100"}`}>Team</Link></div></div>
    {GROUPS.map(group => {
